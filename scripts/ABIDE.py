""" Script for PNN experiment in the journal paper """
...
def main(cfg: DictConfig):
    # Read data 
    X = np.load('data/important/ABIDE/X.npy')
    ...
    match solver:
        ...
        case 'SCOP':
            ...
            w_, _, _, _, O_ , _ = solver_.SCGL_initialization(X)
            L_hat = O_.T @ LKron(w_, V, d) @ O_
        ...
        case 'SDP':
            ...
            solver_.cross_validation(X, verbose = 0)
            L_hat = solver_.solve(C, verbose = 1)

        case 'SLGP':
            # SLGP Graph estimation
            L_hat = SmoothSheafDiffusion(X, V, d, int(1.1 * np.log(V) * (V - 1) / 2)).LaplacianBuilder()

    uuid : str = f'ABIDE_{solver}.npy'
    SAVE_DIR : Path = Path('.') / 'data/interim/ABIDE/' / uuid
