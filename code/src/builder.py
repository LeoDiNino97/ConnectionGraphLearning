...
# class GridCG():
#     '''
#     This class implement a grid with a weighted consistent connection graph built on top of it

#     INPUT:
#         side: int. The dimension of the side of the grid
#         d: int. The dimension of the stalks over the nodes
#         consistent: bool. A flag for the connection graph to be consistent
#         special: bool. A flag for the maps to be sampled from the SO(d) group
...
#         self.d = d
#         self.V = side ** 2
#         self.k = 1  # Number of connected components
#         self.q = self.V - 1
...
#         self.A , self.edges = self.Adjacency()
...
#         W = np.zeros_like(self.A, dtype=float)
#         for edge in self.edges:
#             w = np.random.uniform(low=0.1, high=3)
#             W[edge[0], edge[1]] = w
#             W[edge[1], edge[0]] = w
#         return W
...
#         return CochainSample(X=X, covariance=covariance)
...
#                         G.add_edge(i, j, weight=-self.L[i, j])  
#         pos = {
#             node: (node % self.side, -(node // self.side))
#             for node in range(self.V)
#         }
